...
function showData(data) {
    ...
    const svg = d3.select('#fra-nat-graph03 .graph')
        .append("svg")
    ...
    // Écriture du titre
    const title = d3.select('#fra-nat-graph03 .graph-title')
        .html("Evolution des admisssions à l'hôpital des patients Covid")
        .style('padding-right', paddingTitles + "px")
        .style('padding-left', paddingTitles + "px");

    // Écriture du sous-titre
    const subtitle = d3.select('#fra-nat-graph03 .graph-subtitle')
        .html('depuis mars 2020')
        .style('padding-right', paddingTitles + "px")
        .style('padding-left', paddingTitles + "px");

    // Écriture
    const caption = d3.select('#fra-nat-graph03 .graph-caption')
    ...
    if (deviceType() == "desktop") {
        ...
        rect.on("mouseover", function (d) {
            ...
            tooltip.append("text")
                .attr("x", 5)
                .attr("y", 32)
                .text(`Moyenne lissée: ${Math.round(d.roll_hosp).toLocaleString('fr-FR')}`)
                .attr("font-size", "10px")
